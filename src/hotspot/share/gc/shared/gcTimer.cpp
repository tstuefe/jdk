...
void TimePartitions::report_gc_phase_start(const char* name, const Ticks& time, GCPhase::PhaseType type) {
  ...
  // With Shenandoah, skip this check if we do high frequency uncommit or trimming.
  avoid_phase_length_check |= (UseShenandoahGC && GCTrimNativeHeap && GCTrimNativeHeapDelay < 1);
#endif

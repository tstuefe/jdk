...
bool VirtualMemoryTracker::add_reserved_region(address base_addr, size_t size,
  ...
  if (reserved_rgn == nullptr) {
    ...
    if (reserved_rgn->same_region(base_addr, size) &&
      ...
      log_error(nmt)("Error: existing region: [" INTPTR_FORMAT "-" INTPTR_FORMAT "), memory tag %u.\n"
      ...
      if (MemTracker::tracking_level() == NMT_detail) {
        log_error(nmt)("Existing region allocated from:");
        reserved_rgn->call_stack()->print_on(tty);
        log_error(nmt)("New region allocated from:");
        stack.print_on(tty);
      }
      ShouldNotReachHere();
      return false;

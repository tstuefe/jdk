    ...
    if (_quiet_period_countdown == 0) {
      ...
      if (AutoTrimNativeHeapIntervalAdaptive) {
        ...
        if (info.have_values) {
          ...
          // Note: values in K
          const ssize_t delta_threshold_k = 1024;
          if (_last_deltas[2] > delta_threshold_k &&
              _last_deltas[1] > delta_threshold_k &&
              _last_deltas[0] > delta_threshold_k) {
            _last_deltas[0] = _last_deltas[1] = _last_deltas[2] = 0;
            _quiet_period_countdown = 10;
          }
        }
      }
    } else {
